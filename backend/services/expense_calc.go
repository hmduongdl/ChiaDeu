...
// chia trước khi ghi vào database.
func ValidateExpense(expense models.Expense, splits []models.ExpenseSplit) error {
	if expense.AmountMinor <= 0 {
		return ErrInvalidAmount
	}
	if expense.PaidBy == "" {
		return errors.New("paid_by là bắt buộc")
	}
	switch expense.SplitType {
	case models.SplitTypeEqual, models.SplitTypePercent, models.SplitTypeWeight, models.SplitTypeCustom:
	default:
		return fmt.Errorf("kiểu chia %q không hợp lệ", expense.SplitType)
	}
	if len(splits) == 0 {
		return errors.New("cần ít nhất một phần chia")
	}
	for _, split := range splits {
		if split.UserID == "" {
			return errors.New("phần chia thiếu user_id")
		}
		if split.ShareMinor < 0 {
			return errors.New("phần chia không được âm")
		}
	}
	if SumSplits(splits) != expense.AmountMinor {
		return ErrSplitsMismatch
	}
	return nil
}
